...
contract LicenseStake {
    ...
    function stake(uint8 tier, address referrer) external nonReentrant returns (uint256 index) {
        ...
        // interactions last (CEI). SIMN-style tokens may omit the bool return.
        _safeTransferFrom(msg.sender, address(this), price);
        if (treasuryFee > 0) _safeTransfer(treasury, treasuryFee);
        if (referrerFee > 0) _safeTransfer(referrer, referrerFee);
        if (rebate > 0) _safeTransfer(msg.sender, rebate);
    }
    ...
    function _open(
        ...
        require(principal <= type(uint128).max, "principal overflow");
    ...
    function unstake(address beneficiary, uint256 index) external nonReentrant {
        ...
        _safeTransfer(p.funder, refund);
    }

    /// Accepts standard ERC-20s and tokens that return no data from transfer.
